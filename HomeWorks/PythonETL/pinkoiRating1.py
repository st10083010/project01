...
import requests, json, time , tqdm, os
...
for i in range(0,3):
    ...
    for itemURL in itemInfor:
        ...
        if itemInforDict['@type'] == "Product" :
            ratingURL = 'https://www.pinkoi.com/apiv2/review/get_item_reviews?tid={}&limit=3&product_page=true&sort_by=has_image&order=desc&to_locale=origin'
...
# #----------------------------------------------------------------------------------------------
            ratingRes = requests.get(ratingURL.format(itemInforDict['productID']))
            ratingJson = json.loads(ratingRes.text)
